...
# Thinking-режим не поддерживает tool_choice="required" и замедляет ответы
...
def analyze_recent_meals(entries: list[dict], previous_comments: list[str] | None = None) -> str:
    ...
    target_entry = food_entries[-1]
    context_entries = food_entries[:-1]
    target_text = _format_entries_text([target_entry])
    context_text = _format_entries_text(context_entries) if context_entries else ""

    system_prompt = (
        f"{rules}\n\n"
        "Тебе дают ОДИН приём пищи, на который нужно дать отзыв, "
        "и список более ранних приёмов пищи за тот же день — это контекст. "
        "Комментируй ТОЛЬКО целевой приём (1-2 предложения), "
        "контекст используй только чтобы учитывать накопительный баланс рациона "
        "(белок/клетчатка/овощи и т.п.). НЕ комментируй контекстные приёмы как отдельные.\n\n"
        "Следуй формату ответа на приём пищи из инструкции выше.\n\n"
        "Важно: если в контексте «Поздний ужин» стоит ПЕРЕД любым другим приёмом "
        "пищи дня (Ранний завтрак, Завтрак, Ланч, Обед, Полдник, Ужин) — это "
        "ужин с прошлой ночи (после полуночи), он относится к ПРЕДЫДУЩЕМУ дню. "
        "Полностью игнорируй такую запись — не учитывай её ни в балансе рациона, "
        "ни в комментариях. Пример: список [Поздний ужин: кефир, Обед: салат, "
        "Ужин: курица] — кефир относится к прошлому дню, его игнорируем.\n\n"
    ...
    if context_text:
        user_content = (
            f"Контекст — более ранние приёмы пищи за день (НЕ комментируй их):\n{context_text}\n\n"
            f"Целевой приём пищи (комментируй именно его):\n{target_text}"
        )
    else:
        user_content = f"Целевой приём пищи (первая запись за день, контекста нет):\n{target_text}"
    if previous_comments:
        comments_text = "\n".join(f"- {c}" for c in previous_comments)
        user_content += f"\n\nТвои предыдущие комментарии за этот день (НЕ повторяй их):\n{comments_text}"
...
def generate_daily_nutrition_report(entries: list[dict]) -> str:
    ...
    system_prompt = (
        ...
        "Важно: «Поздний ужин», стоящий в списке ПЕРЕД любым другим приёмом пищи "
        "дня (Ранний завтрак, Завтрак, Ланч, Обед, Полдник, Ужин) — это ужин с "
        "прошлой ночи (после полуночи), он относится к ПРЕДЫДУЩЕМУ дню. Полностью "
        "исключи такую запись из анализа — не упоминай её, не учитывай в балансе "
        "рациона, в рекомендациях про неё не пиши. Пример: [Поздний ужин: кефир, "
        "Обед: салат, Ужин: курица] — кефир из прошлой ночи, игнорируем; "
        "разбираем только обед и ужин. «Поздний ужин» в конце списка, после "
        "других приёмов — нормальный ужин этого дня, учитывай его."
    )
...
def generate_weekly_nutrition_report(entries: list[dict]) -> str:
    ...
    system_prompt = (
        ...
        "Важно: в рамках одного дня «Поздний ужин», стоящий в списке ПЕРЕД "
        "любым другим приёмом пищи дня (Ранний завтрак, Завтрак, Ланч, Обед, "
        "Полдник, Ужин), — это ужин с прошлой ночи (после полуночи), он "
        "относится к предыдущему дню. Полностью исключай такие записи из "
        "анализа. «Поздний ужин» в конце дня, после других приёмов — "
        "нормальный."
    )
